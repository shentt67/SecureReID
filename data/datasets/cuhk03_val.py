...
class CUHK03VAL(BaseImageDataset):
    """
    CUHK03
    Reference:
    ...
    def __init__(self, root='./toDataset', split_id=0, cuhk03_labeled=False,
        ...
        self.split_classic_det_json_path = osp.join(self.dataset_dir, 'val_splits_classic_detected.json')
        self.split_classic_lab_json_path = osp.join(self.dataset_dir, 'val_splits_classic_labeled.json')

        self.split_new_det_json_path = osp.join(self.dataset_dir, 'val_splits_new_detected.json')
        self.split_new_lab_json_path = osp.join(self.dataset_dir, 'val_splits_new_labeled.json')

        self.split_new_det_mat_path = osp.join(self.dataset_dir, 'cuhk03_new_protocol_config_detected_val.mat')
        self.split_new_lab_mat_path = osp.join(self.dataset_dir, 'cuhk03_new_protocol_config_labeled_val.mat')
        ...
        val_query = split['val_query']
        val_gallery = split['val_gallery']

        if verbose:
            print("=> CUHK03_VAL ({}) loaded".format(image_type))
    ...
    def _preprocess(self):
        ...
        def _extract_new_split(split_dict, img_dir):
            ...
            val_query_idxs = split_dict['val_query_idx'].flatten() - 1
            val_gallery_idxs = split_dict['val_gallery_idx'].flatten() - 1
            ...
            val_query_info = _extract_set(filelist, pids, pid2label, val_query_idxs, img_dir, relabel=False)
            val_gallery_info = _extract_set(filelist, pids, pid2label, val_gallery_idxs, img_dir, relabel=False)
            return train_info, query_info, gallery_info, val_query_info, val_gallery_info
        ...
        train_info, query_info, gallery_info, val_query_info, val_gallery_info = _extract_new_split(
            loadmat(self.split_new_det_mat_path),
            self.imgs_detected_dir,
        )
        splits = [{
            'train': train_info[0], 'query': query_info[0], 'gallery': gallery_info[0],
            'val_query': val_query_info[0], 'val_gallery': val_gallery_info[0],
            ...
            'num_val_query_pids': val_query_info[1], 'num_val_query_imgs': val_query_info[2],
            'num_val_gallery_pids': val_gallery_info[1], 'num_val_gallery_imgs': val_gallery_info[2],
        }]
