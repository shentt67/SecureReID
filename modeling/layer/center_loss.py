...
class CenterLoss(nn.Module):
    ...
    def __init__(self, num_classes=751, feat_dim=2048, use_gpu=True):
        ...
        if self.use_gpu:
            ...
            self.centers = nn.Parameter(torch.randn(self.num_classes, self.feat_dim).to("cuda:0"))
        else:
    ...
    def forward(self, x, labels):
        ...
        if self.use_gpu: classes = classes.to("cuda:0")
...
if __name__ == '__main__':
    ...
    if use_gpu:
        ...
        features = torch.rand(16, 2048).to("cuda:0")
        # targets = torch.Tensor([0, 1, 2, 3, 2, 3, 1, 4, 5, 3, 2, 1, 0, 0, 5, 4]).cuda()
        targets = torch.Tensor([0, 1, 2, 3, 2, 3, 1, 4, 5, 3, 2, 1, 0, 0, 5, 4]).to("cuda:0")
